...
PORTFOLIO_FILE = Path.home() / ".openclaw" / "data" / "portfolio.json"
...
def show_portfolio(output_json=False):
    """显示持仓"""
...
def analyze_portfolio(output_json=False):
    """分析所有持仓"""
    portfolio = load_portfolio()
    positions = portfolio.get("positions", [])
...
def main():
    ...
    # show
    show_parser = subparsers.add_parser("show", help="显示持仓")
    ...
    analyze_parser.add_argument("--json", "-j", action="store_true", help="JSON输出")
    
    args = parser.parse_args()
    
    if args.command == "show":
        show_portfolio(output_json=getattr(args, 'json', False))
        ...
        result = analyze_portfolio(output_json=is_json)
        if is_json:
